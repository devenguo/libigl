...
int main(int argc, char *argv[])
{
  ...
  cube_size = upper_bound - lower_bound;
  std::cout << cube_size(0) << " " << cube_size(1) << " " << cube_size(2) << std::endl;
  igl::writeOBJ("./voxels/skin.obj",V,F);
  Eigen::MatrixXd normals;
  igl::per_vertex_normals(V, F, normals);
  V = V + 0.01*normals;
  igl::writeOBJ("./voxels/skin_fat.obj",V,F);
  ...
  Eigen::MatrixXd _Q = Eigen::MatrixXd::Random(1000000,3);
  const Eigen::RowVector3d Qmin = _Q.colwise().minCoeff();
  const Eigen::RowVector3d Qmax = _Q.colwise().maxCoeff();
  const Eigen::RowVector3d Qdiag = Qmax-Qmin;
  std::cout << Qmin(0) << " " << Qmin(1) << " " << Qmin(2) << std::endl;
  std::cout << Qmax(0) << " " << Qmax(1) << " " << Qmax(2) << std::endl;
  std::cout << Qdiag(0) << " " << Qdiag(1) << " " << Qdiag(2) << std::endl;
  
  int k = bound / d + 1;
  Eigen::MatrixXd Q(k * k * k, 3);
  int cnt = 0;
  d *= 2. / bound;
  double bound_lower = -1., bound_upper = 1.;
  for(double i = bound_lower; i <= bound_upper; i += d) {
    for(double j = bound_lower; j <= bound_upper; j += d) {
      for(double k = bound_lower; k <= bound_upper; k += d) {
        Q.row(cnt) = Eigen::Vector3d(i, j, k);
        cnt += 1;
      }
    }
  }
  // std::cout << cnt << " " << k*k*k << std::endl;
  ...
  // std::cout << Vmin(0) << " " << Vmin(1) << " " << Vmin(2) << std::endl;
  ...
  Eigen::MatrixXd QiP;
  {
    ...
    igl::writeOBJ("./voxels/points.obj", QiP, _);
  }
